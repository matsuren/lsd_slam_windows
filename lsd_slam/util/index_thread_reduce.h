...
public:
	inline IndexThreadReduce()
	{
		...
		maxIndex = 0;
		stepSize = 1;
		callPerIndex = std::bind(&IndexThreadReduce::callPerIndexDefault, this, _1, _2, _3);
	...
	inline void reduce(std::function<void(int,int,RunningStats*)> callPerIndex, int first, int end, int stepSize = 0)
	{
		...
		maxIndex = 0;
		using namespace std::placeholders;
		this->callPerIndex = std::bind(&IndexThreadReduce::callPerIndexDefault, this, _1, _2, _3);
